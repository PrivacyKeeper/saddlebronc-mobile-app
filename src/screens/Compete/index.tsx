...
export function CompeteScreen() {
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase.from('saddlebronc_runs').insert(payload);
    setSaving(false);
    if (error) {
  ...
  return (
    <ScrollView style={cs.container} contentContainerStyle={cs.content}>
      ...
      <Text style={cs.sub}>
        Hand-timed saddle bronc riding runs stay yours — they are structurally separated from official results and never reach a
        leaderboard.
      </Text>
      ...
      {loading ? (
        ...
        <Text style={cs.empty}>Nothing logged yet. Log your first saddle bronc riding run above.</Text>
      ) : (
        runs.map((run) => (
