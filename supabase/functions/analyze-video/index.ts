...
// analyze-video edge function for saddle bronc riding.
//
...
const ANALYSIS_SCHEMA = {
  ...
  properties: {
    ...
    phases: {
      type: "object",
      additionalProperties: false,
      required: ["mark_out", "spur_stroke", "timing", "control"],
      properties: {
          mark_out: { type: "string" },
          spur_stroke: { type: "string" },
          timing: { type: "string" },
          control: { type: "string" },
      },
    },
...
const SYSTEM_PROMPT =
  "You are an expert saddle bronc riding coach. Analyze the athlete's run from the supplied frames. " +
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: SYSTEM_PROMPT + " JSON schema: " + JSON.stringify(ANALYSIS_SCHEMA) },
          {
            role: "user",
            content: [
              { type: "text", text: "Analyze this saddle bronc riding run." },
              ...images,
            ],
          },
        ],
    ...
    if (supabaseUrl && serviceKey && user_id) {
      ...
      await admin.from("video_analysis_results").insert({
        user_id,
        event_type: body.event_type ?? "saddlebronc",
        video_url,
