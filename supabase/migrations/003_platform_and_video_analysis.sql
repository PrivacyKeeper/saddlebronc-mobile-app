-- 003 platform layer + saddlebronc_runs for saddlebronc-mobile-app
...
-- Discipline run log for saddle bronc riding.
create table if not exists public.saddlebronc_runs (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
...
create index if not exists saddlebronc_runs_user_idx on public.saddlebronc_runs (user_id, created_at desc);

alter table public.saddlebronc_runs enable row level security;

drop policy if exists "saddlebronc_runs_owner_all" on public.saddlebronc_runs;
create policy "saddlebronc_runs_owner_all" on public.saddlebronc_runs
